...
final case class SparkContext(underlying: UnderlyingSparkContext) { self =>
  ...
  def accumulable[R, T](initialValue: R)(implicit param: AccumulableParam[R, T]): Accumulable[R, T] =
    get(_.accumulable[R, T](initialValue))
  ...
  def accumulable[R, T](initialValue: R, name: String)(implicit param: AccumulableParam[R, T]): Accumulable[R, T] =
    get(_.accumulable[R, T](initialValue, name))
  ...
  def accumulableCollection[R <% Growable[T] with TraversableOnce[T] with Serializable: ClassTag, T](
      initialValue: R
  ): Accumulable[R, T] = get(_.accumulableCollection[R, T](initialValue))
  ...
  def accumulator[T](initialValue: T)(implicit param: AccumulatorParam[T]): Accumulator[T] =
    get(_.accumulator[T](initialValue))
  ...
  def accumulator[T](initialValue: T, name: String)(implicit param: AccumulatorParam[T]): Accumulator[T] =
    get(_.accumulator[T](initialValue, name))
   ...
   */
  def cancelJobGroup(groupId: String) = get(_.cancelJobGroup(groupId))
   ...
   * actions and RDDs.
   */
  def setCallSite(shortCallSite: String) = get(_.setCallSite(shortCallSite))
   ...
   */
  def setCheckpointDir(directory: String) = get(_.setCheckpointDir(directory))

  /** Set a human readable description of the current job. */
  def setJobDescription(value: String) = get(_.setJobDescription(value))
   ...
   */
  def setLocalProperty(key: String, value: String) = get(_.setLocalProperty(key, value))
   ...
   */
  def setLogLevel(logLevel: String) = get(_.setLogLevel(logLevel))
