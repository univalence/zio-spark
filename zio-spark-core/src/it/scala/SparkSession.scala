...
final case class SparkSession(underlying: UnderlyingSparkSession) { self =>
  ...
  def sql(sqlText: String)(implicit trace: ZTraceElement): Task[DataFrame] =
    ZIO.attemptBlocking(Dataset(underlying.sql(sqlText)))

  def conf: Conf =
    new Conf {
      override def getAll: UIO[Map[String, String]] = UIO.succeed(underlying.conf.getAll)
    }
}
...
object SparkSession extends Accessible[SparkSession] {
  ...
  def attempt[Out](f: UnderlyingSparkSession => Out)(implicit trace: ZTraceElement): SIO[Out] =
    ZIO.serviceWithZIO[SparkSession](ss => ZIO.attempt(f(ss.underlying))(trace))
