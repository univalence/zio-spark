...
class TemplateWithComments(template: Template, filterOverlay: Boolean) {
  ...
  private lazy val startTemplate: Option[Position] =
    commentTokens.find { case (content, _) => content.contains("template:on") }.map(_._2)
  private lazy val endTemplate: Option[Position] =
    commentTokens.find { case (content, _) => content.contains("template:off") }.map(_._2)
