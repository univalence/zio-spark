  ...
  def dataFrameReaderReadingSpec: Spec[SparkSession, Throwable] =
    suite("DataFrameReader reading files")(
      ...
      test("DataFrameReader can read a JSON file") {
        for {
          df     <- SparkSession.read.option("multiline", "true").json(resourcesPath("data-json"))
          output <- df.count
        } yield assertTrue(output == 4L)
      },
      test("DataFrameReader can read a Parquet file") {
        for {
          df     <- SparkSession.read.parquet(resourcesPath("data-parquet"))
          output <- df.count
        } yield assertTrue(output == 4L)
      },
      test("DataFrameReader can read a Orc file") {
        for {
          df     <- SparkSession.read.orc(resourcesPath("data-orc"))
          output <- df.count
        } yield assertTrue(output == 4L)
      },
      test("DataFrameReader can read a Text file") {
        for {
          df     <- SparkSession.read.textFile(resourcesPath("data-txt"))
      ...
      test("DataFrameReader can have a schema by default") {
        ...
        for {
          df <- SparkSession.read.option("multiline", "true").schema(schema).json(resourcesPath("data-json"))
